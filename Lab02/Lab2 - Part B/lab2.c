...
void intPortG(){
  ...
  //If Pin 2 has a 0, it increments the count.
  if(GPIOPinRead(GPIO_PORTG_BASE, GPIO_PIN_2)==0x00){
    count++;
  }
  
  //If Pin 1 has a 0, it decrements the count.
  if(GPIOPinRead(GPIO_PORTG_BASE, GPIO_PIN_1)==0x00){
    count--;
  }
}
...
int main(void){
    ...
    */
    ...
    GPIOIntTypeSet(GPIO_PORTG_BASE, (GPIO_PIN_2 | GPIO_PIN_1), GPIO_FALLING_EDGE);
